> Adapted and modified by abhi-zit77, 2026-09-22, from [usestrix/strix `strix/skills/frameworks/nextjs.md`](https://github.com/usestrix/strix/blob/56e9ae982c2fdd00c7c0b9afc49af035470dd310/strix/skills/frameworks/nextjs.md).
...
- Inspect the target version's `middleware.ts` or `proxy.ts` convention; do not assume the filename or runtime.
...
```javascript
// Optional Pages Router hints; absence is not evidence that there are no routes.
const manifest = window.__BUILD_MANIFEST;
if (manifest?.sortedPages) console.log(manifest.sortedPages);
const data = document.getElementById('__NEXT_DATA__');
if (data?.textContent) console.log(JSON.parse(data.textContent).page);
```

Enumerate actual navigation, source routes and observed network requests for App
Router coverage. Inspect delivered bundles for public environment values; do not
enumerate `process.env` in a browser. Public variables are inlined at build time
and dynamic lookup is not a reliable inventory. Redact any secrets found.
...
```
Chunk names can suggest routes but are not an authoritative route map; confirm
using source or observed behavior. Probe observed artifact paths rather than
assuming a directory listing or every illustrated path exists.
